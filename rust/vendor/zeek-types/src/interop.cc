...
std::unique_ptr<zeek::ValPtr> make_vector(std::unique_ptr<ValPtrVector> xs) {
  ...
  for (size_t i = 0; i < vals.size(); ++i) {
    result->Assign(i, std::move(vals[i]));
  }
...
std::unique_ptr<zeek::ValPtr> make_pattern(rust::Str exact_,
                                           rust::Str anywhere_) {
  auto exact = std::unique_ptr<char[]>(new char[exact_.size()]);
  std::ranges::copy(exact_, exact.get());

  auto anywhere = std::unique_ptr<char[]>(new char[anywhere_.size()]);
  std::ranges::copy(anywhere_, anywhere.get());

  auto re =
      std::make_unique<zeek::RE_Matcher>(exact.release(), anywhere.release());
